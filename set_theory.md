...
注意到目前构造的集合都是无序的，其实配对公理也可以构造有序对。给定集合$$x,y$$，我们构造二元**有序对**$$(x,y):=\{\{x\},\{x,y\}\}$$，则有$$(x,y)=(u,v)⇔x=u∧y=v$$（为什么）。
...
正则性公理说如果集合𝒞非空，则存在𝒞中的一个元素A，使得A与𝒞不包含同一个元素。正则性公理保证了不存在无穷递降的集合链，即不存在$$x_1∋x_2∋x_3∋\cdots$$。因此，也不存在循环集合链，即不存在$$x_1∋x_2∋\cdots∋x_n∋x_1$$。
...
2. 请比较
  ...
  * $$∃x:(x∈A→P(x,A))$$
  * $$∃x:(x∈A∧P(x,A))$$，通常记作$$∃x∈A:P(x,A)$$
3. 尝试构造n元有序组，并证明正确性（两个有序组相等当且仅当逐分量相等）。
4. 定义自然数ℕ上的加法+满足：(i)$$∀a∈ℕ:a+0=a$$；(ii)$$∀a,b∈ℕ:a+S(b)=S(a+b)$$，请证明自然数上的加法具有交换律和结合律。类似请定义自然数ℕ上的乘法×，证明其交换律和结合律，以及对加法的分配律。
5. 请用分离公理和外延公理证明$$∃!∅:∀x:¬(x∈∅)$$,∅称为**空集**。
6. 有的作者将“存在唯一空集”被作为空集公理添加到公理集合论中，请证明在该理论下，分离公理和替代公理等价。
7. 事实上，配对公理在ZF中是不独立的、冗余的，请用替代公理将其导出。
8. 请用正则性公理证明以下定理：
  * $$∀x:¬(x∈x)$$
  * $$∀x,y:¬(x∈y∧y∈x)$$
  * $$∀x,y,z:¬(x∈y∧y∈z∧z∈x)$$