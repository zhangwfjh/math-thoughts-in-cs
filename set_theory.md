...
3. 定义有限集合$$A$$的**势**为$$A$$的元素个数，记作$$|A|$$，证明$$|𝓟(A)|=2^{|A|}$$。因此幂集$$𝓟(A)$$也记作$$2^A$$。
4. 证明集合相等$$=$$是等价关系，集合包含$$\subseteq$$是偏序关系。
5. 定义$$X$$上的两个关系$$R,S$$的复合关系$$T$$满足$$xTz\leftrightarrow\exists y: xRy \wedge ySz$$，记作$$T=E \circ F$$。思考复合是否保留以下性质(即若R,S均含有某性质，则T也含有该性质):(i)自反性(ii)对称性(iii)传递性(iv)反对称性。
6. 假设有两个全序集$$(A,⩽), (B,⩽)$$，定义$$(A\times B,⩽)$$满足$$(a,b)⩽(c,d)\leftrightarrow a < c \vee (a=c \wedge b⩽d)$$，证明$$(A\times B,⩽)$$也是全序集，称为**字典序**。
7. 设有函数$$f:A\to B$$，令$$X\subseteq A, Y\subseteq B$$，请证明：
  * $$X\subseteq f^{-1}(f(X))$$，等号成立当且仅当$$f$$是单射
  * $$f(f^{-1}(Y))\subseteq Y$$，等号成立当且仅当$$f$$是满射
8. 设有函数$$f:A\to B$$，且$$X\subseteq Y\subseteq A$$，请证明：
  * $$f(X)\subseteq f(Y)$$
  * $$f(X\cup Y)=f(X)\cup f(Y)$$
  * $$f(X\cap Y)\subseteq f(X)\cap f(Y)$$，等号成立当且仅当$$f$$是单射
  * $$f(X\setminus Y)\supseteq f(X)\setminus f(Y)$$，等号成立当且仅当$$f$$是满射
9. 设有函数$$f:A\to B$$，且$$X\subseteq Y\subseteq B$$，请证明：
...
计算机中并没有集合的概念，为了赋予计算机对于集合的表达和运算能力，我们必须从基本的数据结构和程序逻辑开始将其构造出来。前面我们已经介绍了如何从数学上进行构造和运算，因此我们只需谨慎的将其用程序语言表达出来。

### 集合的构造

一个集合最直接的构造方式是从空集开始，依次向其中添加元素。在计算机中，数组是内存中一块连续的空间，我们可以用数组来存储集合的元素。例如
```
julia> A = [];
