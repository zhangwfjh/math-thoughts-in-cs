...
不难验证，交集、并集都有交换律和结合律，并且交集对并集有结合律。实际上，根据运算的定义，这些性质都是由逻辑导出的。因此，布尔代数所有的性质均能在集合代数中体现。
...
从ZF公理出发，我们定义二元关系R为两个集合X,Y的笛卡尔积的一个子集，即：$$R \subseteq X \times Y$$。如果$$x \in X, y \in Y$$满足$$(x,y) \in R$$，则我们记$$xRy$$。例如对于$$X=Y=\{1,2,3\}$$，我们可以定义相等关系$$R_= =\{(1,1),(2,2),(3,3)\}$$和小于关系$$R_< =\{(1,2),(2,3),(1,3)\}$$。
...
* 完全性:$$xRy \vee yRx$$
* 三歧性:$$xRy \vee yRx \vee x=y$$

如果一个关系R同时满足自反性、对称性、传递性，我们称R为**等价关系**，例如=是等价关系（为什么）。如果一个关系R同时满足自反性、反对称性、传递性，则称R为**偏序关系**，例如≤是偏序关系（为什么）。
...
5. 定义$$X$$上的两个关系$$R,S$$的复合关系$$T$$满足$$xTz\leftrightarrow\exists y: xRy \wedge ySz$$，记作$$T=E \circ F$$。思考复合是否保留以下性质(即若R,S均含有某性质，则T也含有该性质):(i)自反性(ii)对称性(iii)传递性。
6. 
