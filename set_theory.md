...
并集公理定义了一族集合的并集A为每个𝒞中集合B的元素x构成的集合，记作$$A=⋃𝒞$$。
...
其中，$$P(x,y,\vec v, A)$$是谓词公式，$$\vec v$$是一族自由变量。注意，$$∃!$$表示唯一存在，这限制了谓词公式P是一个A映射到B的（偏）函数。

替代公理允许通过（偏）函数构造值域集合，集存在集合$$B=F(A;\vec v)=\{y \mid y=F(x; \vec v) \wedge x \in A\}$$ 。其中，$$\forall x:\exists! y:(y=F(x;\vec v) \leftrightarrow P(x,y,\vec v,A))$$。
...
在上一节中，我们主要介绍了ZF理论，描述如何通过谓词逻辑构造合法的集合。在这一节中，我们将定义一些集合的基本运算。这些运算使得我们可以忘记谓词逻辑构造符合直观的合法的新集合。

### 基本运算

假设有集合$$A, B$$，我们定义：

* 并集：$$A∪B=\{x|x \in A \vee x\in B\}$$
* 交集：$$A∩B=\{x|x \in A \wedge x\in B\}$$
* 差集：$$A \setminus B = \{x|x \in A \wedge x \not\in B\}$$
* 包含：$$A⊆B⇔∀x:(x∈A→x∈B)$$
* 相等：$$A=B⇔∀x:(x∈A↔x∈B)$$

不难验证，交集、并集都有交换律、结合律和分配律(为什么)。实际上，根据运算的定义，这些性质都是由逻辑导出的。因此，布尔代数所有的性质均能在集合代数中体现。

给定任意集合$$A$$，记$$𝓤=𝓟(A)$$，我们做如下对应：

* 0元算子: $$\emptyset \longleftrightarrow \bot, 𝓤 \longleftrightarrow \top$$
* 1元算子: $$𝓤 \setminus X \longleftrightarrow \neg x$$
* 2元算子: $$X \cup Y \longleftrightarrow x \vee y, X \cap Y \longleftrightarrow x \wedge y$$

因此我们说，关于A的集合代数构成了布尔代数，这个对应称作一个**代数同态**。

### 关系与函数

在数学以及计算机科学中，关系是最重要也是最频繁用到的一个概念。例如，两个集合相等，一个数大于另一个数，一个文件放在一个文件夹内，一个程序在另一个程序之后执行等，都属于关系。确切来说，关系指的是两个或多个对象之间存在某种联系，是其他对象之间不存在的。本节中，我们主要讨论二元关系。

#### 二元关系

从ZF公理出发，我们定义二元关系R为两个集合X,Y的笛卡尔积的一个子集，即：$$R \subseteq X \times Y$$。如果$$x \in X, y \in Y$$满足$$(x,y) \in R$$，则我们记$$xRy$$。例如对于$$X=Y=\{1,2,3\}$$，我们可以定义相等关系$$R _ = =\{(1,1),(2,2),(3,3)\}$$和小于关系$$R_< =\{(1,2),(2,3),(1,3)\}$$。

对于不同的集合X,Y，则定义在上面的关系也不相同。但是，不同的关系可以具有相同的一些性质。我们列出几个较为重要的性质：

* 自反性:$$xRx$$
* 对称性:$$xRy \leftrightarrow yRx$$
* 反对称性:$$xRy \wedge yRx \leftrightarrow x=y$$
* 传递性:$$xRy \wedge yRz \to xRz$$
* 完全性:$$xRy \vee yRx$$

如果一个关系R同时满足自反性、对称性、传递性，我们称R为**等价关系**，例如=是等价关系。如果一个关系R同时满足自反性、反对称性、传递性，则称R为**偏序关系**，例如⩽是偏序关系。如果一个偏序R还满足完全性，则我们称R为一个**全序**或**线性序**。

#### 函数

函数也可以看作一种特殊的二元关系$$f$$，满足$$\forall x \in X, \exists! y \in Y:xfy$$，记作$$y=f(x)$$，或者$$f:x \mapsto y$$。我们将$$X$$称为**定义域**，$$Y$$称为**到达域**，称$$Z=\{y \in Y|y=f(x)\}$$为**值域**。函数也有一些重要的性质：

* 单射:$$\forall x_1,x_2 \in X: (f(x_1)=f(x_2) \to x_1=x_2)$$
* 满射:$$\forall y \in Y: \exists x \in X:y=f(x)$$
* 双射:$$\forall y \in Y: \exists! x \in X:y=f(x)$$

注意到，一个函数$$f$$为双射当且仅当$$f$$同时是单射和满射。如果$$f$$为单射，我们可以定义它的**逆函数**$$f^{-1}$$满足$$\forall x,y:(y=f(x)\leftrightarrow x=f^{-1}(y)$$。

对于任意函数$$f:A\to B$$，定义在集合上的延拓$$\bar f:2^A\to 2^B$$满足$$\forall X\subseteq A:\bar f(X)=\{y\in B\mid \exists x\in X:y=f(x)\}$$，以及$$\bar f^{-1}:2^B\to2^A$$满足$$\forall Y\subseteq B:\bar f^{-1}(Y)=\{x\in A\mid \exists y\in Y:y=f(x)\}$$。我们称$$\bar f(X)$$为$$X$$的**像**，$$\bar f^{-1}(Y)$$为$$Y$$的**原像**或**纤维**。通常在无歧义的情况下，我们也记作$$f(X), f^{-1}(Y)$$。

### 练习

1. 设$$A,B⊆𝓤$$都是集合，定义**补集**$$A^C=𝓤\setminus A$$，请证明
  * 幂等律：$$A\cup A = A, A\cap A = A$$
  * 吸收律：$$A\cup(A\cap B)=A, A\cap(A\cup B)=A$$
  * De Morgan:$$(A\cup B)^C=A^C\cap B^C, (A\cap B)^C=A^C\cup B^C$$
  * 差集与交集：$$A\setminus B=A\cap B^C, A\cap B=A\setminus(A\setminus B)$$
2. 定义$$A_{1j}=\mathbb{R}\times[j-1,j], A_{2i}=[i-1,i]\times\mathbb{R}$$，计算$$\bigcup_{i=1}^2\bigcap_{j=1}^2A_{ij}, \bigcap_{j=1}^2\bigcup_{i=1}^2A_{ij}$$。
3. 证明$$X \times Y ⊆ 𝓟(𝓟(X \bigcup Y))$$。
4. 计算$$𝓟(∅),𝓟(𝓟(∅)),𝓟(𝓟(𝓟(∅)))$$。
5. 对于任意集合$$A\subseteq X$$，定义**示性函数**$$1_A:X\to\{0,1\}$$满足
$$1_A(x):=\begin{cases}1,& x \in A\\ 0,& x \not\in A\end{cases}$$，
请对任意两个集合$$A,B\subseteq X$$，计算$$1_{A\cup B}, 1_{A\cap B}, 1 _ {A\setminus B}$$。

6. 定义有限集合$$A$$的**基数**为$$A$$的元素个数，记作$$|A|$$，证明：
  * $$|𝓟(A)|=2^{|A|}$$（因此幂集$$𝓟(A)$$也记作$$2^A$$）
  * 定义函数$$f:A\to B$$，如果$$f$$是单射，则$$|A|⩽|B|$$；如果$$f$$是满射，则$$|B|⩽|A|$$
7. 证明集合相等$$=$$是等价关系，集合包含$$\subseteq$$是偏序关系。
8. 定义$$X$$上的两个关系$$R,S$$的复合关系$$T$$满足$$xTz\leftrightarrow\exists y: xRy \wedge ySz$$，记作$$T=E \circ F$$。思考复合是否保留以下性质(即若R,S均含有某性质，则T也含有该性质):(i)自反性(ii)对称性(iii)传递性(iv)反对称性。
9. 假设有两个全序集$$(A,⩽), (B,⩽)$$，定义$$(A\times B,⩽)$$满足$$(a,b)⩽(c,d)\leftrightarrow a < c \vee (a=c \wedge b⩽d)$$，证明$$(A\times B,⩽)$$也是全序集，称为**字典序**。
10. 设有函数$$f:A\to B$$，令$$X\subseteq A, Y\subseteq B$$，请证明：
  * $$X\subseteq f^{-1}(f(X))$$，等号成立当且仅当$$f$$是单射
  * $$f(f^{-1}(Y))\subseteq Y$$，等号成立当且仅当$$f$$是满射
11. 设有函数$$f:A\to B$$，且$$X\subseteq Y\subseteq A$$，请证明：
  * $$f(X)\subseteq f(Y)$$
  * $$f(X\cup Y)=f(X)\cup f(Y)$$
  * $$f(X\cap Y)\subseteq f(X)\cap f(Y)$$，等号成立当且仅当$$f$$是单射
  * $$f(X\setminus Y)\supseteq f(X)\setminus f(Y)$$，等号成立当且仅当$$f$$是满射
12. 设有函数$$f:A\to B$$，且$$X\subseteq Y\subseteq B$$，请证明：
...
```
julia> B = falses(8); B[1] = B[2] = B[4] = B[7] = true;
julia> B  # A = {1,2,4,7}
...
如果我们希望表示一个集合$$A\subseteq X$$，注意到$$|𝓟(X)|=2^{|X|}$$，一个子集可以和$$[2^{|X|}]$$(即$$\{1,2,\ldots,2^{|X|}\}$$)中的某一自然数对应。考虑数$$b$$二进制表示，将第$$i$$位为1表示为第$$i$$个元素在子集$$A$$中，这种表示是一种一一对应（为什么）。
```
julia> b = Int8(75);
...
3. 给定集合$$A\subseteq X$$和数$$b \in [2^{|X|}]$$，写程序计算$$A$$的数表示和$$b$$对应的子集。进一步，生成$$X$$的幂集。