...
正则性公理说如果集合𝒞非空，则存在𝒞中的一个元素A，使得A与𝒞不包含同一个元素。正则性公理保证了不存在无穷递降的集合链，即不存在$$x_0∋x_1∋x_2∋\cdots$$。因此，也不存在循环集合链，即不存在$$x_0∋x_1∋\cdots∋x_n∋x_0$$。
...
2. 尝试构造n元有序组，并证明正确性（两个有序组相等当且仅当逐分量相等）。
3. 定义自然数ℕ上的加法+满足：(i)$$∀a∈ℕ:a+0=a$$；(ii)$$∀a,b∈ℕ:a+S(b)=S(a+b)$$，请证明自然数上的加法具有交换律和结合律。类似请定义自然数ℕ上的乘法×，证明其交换律和结合律，以及对加法的分配律。
4. 请用分离公理和外延公理证明$$∃!∅:∀x:¬(x∈∅)$$,∅称为**空集**。
5. 有的作者将“存在唯一空集”被作为空集公理添加到公理集合论中，请证明在该理论下，分离公理和外延公理等价。
6. 事实上，配对公理在ZF中是不独立的、冗余的，请用替代公理将其导出。
7. 请用正则性公理证明以下定理：
  * $$∀x:¬(x∈x)$$
  * $$∀x,y:¬(x∈y∧y∈x)$$
  * $$∀x,y,z:¬(x∈y∧y∈z∧z∈x)$$